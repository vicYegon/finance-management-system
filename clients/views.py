...
class ClientCreateView(LoginRequiredMixin, SuccessMessageMixin, CreateView):
    ...
    fields = ['name', 'mandate', 'account_purpose', 'is_saving_account', 'is_current_account',
              ...
              'hometown', 'county', 'mother_name', 'father_name', 'occupation', 'residential_address', 'residential_city',
              ...
              'nxt_of_kin_contact', 'relationship_with_kin', 'nxt_of_kin_residence', 'nxt_of_kin_region', 'emergency_contact_name', 'emergency_contact_phone',
              'relation_with_emergency_contact', 'emergency_contact_residence', 'emergency_contact_county']
...
def deposit_view(request):
    ...
    if form.is_valid():
        ...
        deposit.client.save()
        messages.success(
            request, 'Client Have Deposited {} KSH.'.format(deposit.amount))
...
def withdrawal_view(request):
    ...
    if form.is_valid():
        ...
        if withdrawal.client.account_balance >= withdrawal.amount:
            ...
            withdrawal.save()
            messages.success(
                request, 'Client have Withdrawn {} KSH.'.format(withdrawal.amount))
            return redirect('clients:transaction_view')
        else:
            messages.info(
...
def transaction_view(request):
    ...
    total_deposits = Deposit.objects.filter(transaction_time=timezone.now().replace(
        month=1, hour=0, minute=0, second=0, microsecond=0)).aggregate(amount__sum=Sum('amount'))['amount__sum']
    total_withdrawals = Withdrawal.objects.filter(transaction_time=timezone.now().replace(
        month=1, hour=0, minute=0, second=0, microsecond=0)).aggregate(amount__sum=Sum('amount'))['amount__sum']
    context = {
        'deposits': deposits,
